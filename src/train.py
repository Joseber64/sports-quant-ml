...
def preprocess(df):
    # Asegurar que FTR existe
    if "FTR" not in df.columns:
        print("Warning: FTR column not found, usando resultado aleatorio")
        df["result"] = np.random.randint(0, 2, len(df))
    else:
        df["result"] = df["FTR"].apply(lambda x: 1 if x == "H" else 0)
    
    # Features básicas disponibles
    ...
    # IMPORTANTE: Imputar NaN ANTES de entrenar
